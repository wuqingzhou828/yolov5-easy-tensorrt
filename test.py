...
def main(opt):
    ...
    for box,score,cls in result:
        box = box.round().int()
        cv2.rectangle(image_copy,box[:2],box[2:4],color=(0,255,255),thickness=3)
    if show:
...
def parse_opt():
    ...
    parser.add_argument('--image', type=str, default="engine/yolov5s.engine", help='save engine path')
    parser.add_argument('--engine', type=str, default="images/bus.jpg", help='test image path')
    parser.add_argument('--device', default='cuda', help='cuda device, i.e. 0 or 0,1,2,3')
