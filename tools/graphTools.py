...
class AdditionNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.convert_matrix = torch.tensor([[1,0,1,0],[0,1,0,1],[-0.5,0,0.5,0],[0,-0.5,0,0.5]],dtype=torch.float32)
    def forward(self,x):
        box = x[:,:,:4]
        conf = x[:,:,4:5]
        score = x[:,:,5:]
...
def registerNMS(gs_graph,detections_per_img=100,score_thresh=0.25,nms_thresh=0.45):
