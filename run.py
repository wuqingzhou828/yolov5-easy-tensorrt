...
from tools import AdditionNet,check_gs,registerNMS,buildEngine
...
def main(opt):
    ...
    gs_graph = registerNMS(gs_graph,detections_per_img,score_thresh,nms_thresh)
    onnx.save(gs.export_onnx(gs_graph), str(onnxsave))
    if not enginesave.exists():
        status = buildEngine(onnxsave,enginesave,workspace,verbose,fp16,noTF32=True)
        if status:
            print(f"Build {enginesave.name} finish")

def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--onnxfile', type=str, default="onnx/yolov5s.onnx", help='origin onnx path')
    parser.add_argument('--onnxsave', type=str, default="onnx/yolov5s_nms.onnx", help='register onnx path')
    parser.add_argument('--enginesave', type=str, default="engine/yolov5s.engine", help='save engine path')

    parser.add_argument('--opset', type=int, default=13, help='opset version')
    parser.add_argument('--GiB', type=int, default=8, help='workspace for engine builder')
    parser.add_argument('--maxObj', type=int, default=100, help='max object in one image')
    parser.add_argument('--score', type=float, default=0.25, help='min score for nms attrs')
    parser.add_argument('--nms_score', type=float, default=0.25, help='min mns_score for nms attrs')

    parser.add_argument('--verbose', action='store_true', help='verbose log print')
    parser.add_argument('--fp16', action='store_true', help='fp16 exporter')
    parser.add_argument('--saveTemp', action='store_true', help='save temp onnx or not')
